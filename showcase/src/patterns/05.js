...
const ClapIcon = ({ style: userStyles = {}, className }) => {
  const { isClicked } = React.useContext(MedumClapContex);
...
const ClapCount = ({ style: userStyles = {}, className }) => {
  const { count, setRef } = React.useContext(MedumClapContex);
...
const CountTotal = ({ style: userStyles = {}, className }) => {
  const { countTotal, setRef } = React.useContext(MedumClapContex);
    ...
    <>
      <div style={{ width: "100%" }}>
        ...
        >
          ...
          <MediumClap.Count
            style={{ color: "green" }}
            className={userCustomStyles.count}
          />
          <MediumClap.Toatl className={userCustomStyles.total} />
        </MediumClap>
        <div className={Styles.info}>{`total clicks ${state.countTotal}`}</div>
      </div>
